...
def finalize_limine_boot(ctx: InstallContext) -> None:
    ...
    if _is_arm_machine():
        ...
        if "cryptdevice=" in cmdline:
            _install_arm_encryption_hooks(ctx)
        kernel_cmdline = ctx.target / "etc" / "kernel" / "cmdline"
        kernel_cmdline.write_text(cmdline + "\n")
...
# Archinstall emits the classic cryptdevice= kernel parameter, so the initramfs
# must use the matching busybox encrypt hook rather than systemd's sd-encrypt.
_omarchy_arm_hooks=()
_omarchy_arm_encrypt_added=false
for _omarchy_arm_hook in "${HOOKS[@]}"; do
  case $_omarchy_arm_hook in
    systemd)
      _omarchy_arm_hooks+=(udev)
      ;;
    sd-vconsole)
      _omarchy_arm_hooks+=(keymap consolefont)
      ;;
    encrypt|sd-encrypt)
      ;;
    filesystems)
      if ! $_omarchy_arm_encrypt_added; then
        _omarchy_arm_hooks+=(encrypt)
...
unset _omarchy_arm_hooks _omarchy_arm_encrypt_added _omarchy_arm_hook
'''


def _install_arm_encryption_hooks(ctx: InstallContext) -> None:
...
def validate_boot(ctx: InstallContext) -> None:
    ...
    if ctx.encrypt and _is_arm_machine():
        ...
        if not encryption_hooks.exists() or "encrypt" not in encryption_hooks.read_text():
