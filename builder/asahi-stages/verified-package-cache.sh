...
prepare_verified_package_cache() {
  ...
  if [[ $OMARCHY_MEDIA_TARGET == aarch64/apple-silicon ]]; then
    printf '%s\n' alsa-ucm-conf-asahi asahi-alarm-keyring asahi-audio asahi-bless asahi-fwextract asahi-scripts grub "${ASAHI_KERNEL_PACKAGE:-linux-asahi}" "${ASAHI_KERNEL_PACKAGE:-linux-asahi}-headers" m1n1 speakersafetyd startup-disk uboot-asahi | filter_target_packages >>"$shipped_base_packages"
